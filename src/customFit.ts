...
export class FitAddon implements ITerminalAddon {
	...
	public proposeDimensions(): ITerminalDimensions | undefined {
		...
		const parentElementStyle = window.getComputedStyle(
			this._terminal.element.parentElement.parentElement
		)
		...
		if (!core._renderCoordinator) return undefined
		const geometry = {
			cols: Math.floor(
				availableWidth / core._renderCoordinator.dimensions.actualCellWidth
			),
			rows: Math.floor(
				availableHeight / core._renderCoordinator.dimensions.actualCellHeight
			)
		}
		return geometry
	}
