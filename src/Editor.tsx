...
export const Editor = ({
  ...
  const setupEditor = () => {
    ...
    console.log(`time to first paint: ${performance.now() - start} milliseconds`);
  ...
  const initWorker = async (view: EditorView) => {
    const innerWorker = new Worker(new URL('./worker.ts', import.meta.url), { type: 'module' });
    ...
    innerWorker.onmessage = async e => {
      if (e.data === 'ready') {
        ...
        console.log(`time for TS worker to load: ${performance.now() - start} milliseconds`);
      }
    };
  };
  ...
  return (
    <>
      ...
      <div id='minimap' class='transition-display' ref={refs.miniMap} />
    </>
  );
};
